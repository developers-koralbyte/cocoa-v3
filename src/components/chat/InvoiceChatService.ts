import { 
  ...
  export const sendInvoiceToChat = async (invoice: Invoice): Promise<string> => {
    try {
      ...
      await updateUserChatsWithInvoiceMessage(chatId, vendorId, buyerId, invoice);
   ...
   */
  const findOrCreateChat = async (vendorId: string, buyerId: string): Promise<string> => {
    try {
      ...
      if (vendorChatsDoc.exists()) {
        ...
        const existingChat = vendorChats.find((chat: any) => chat.receiverId === buyerId);
      ...
      // Check if buyer has a chat with this vendor
      const buyerChatsRef = doc(db, 'userchats', buyerId);
      const buyerChatsDoc = await getDoc(buyerChatsRef);
      
      if (buyerChatsDoc.exists()) {
        const buyerChats = buyerChatsDoc.data().chats || [];
        const existingChat = buyerChats.find((chat: any) => chat.receiverId === vendorId);
   ...
   */
  const createNewChat = async (vendorId: string, buyerId: string): Promise<string> => {
    ...
    // Get vendor and buyer details for userchats
  ...
  const addInvoiceMessageToChat = async (
    ...
    const message = {
      ...
      invoiceId: invoice.id // Reference to the invoice
    };
    ...
    for (const userId of userIds) {
      try {
        ...
        if (userChatsDoc.exists()) {
          ...
          const chatIndex = userChats.findIndex((chat: any) => chat.chatId === chatId);
